...
flowchart TD
    ...
    E90 -- "소시민 지도자를 흠차대신으로 임명" --> COMM["소시민 지도자 광동 파견"]
...
- 엄금론을 채택하고 아편을 엄격히 규제하라는 황제의 결정을 표현한다.
...
- AI는 소시민 이해집단 지도자를 흠차대신으로 임명하는 선택지를 고른다.

선택지 A: 소시민 지도자를 흠차대신으로 임명

- 기본 선택지.
- AI 선택 확률 100%.
- 이벤트 immediate에서 `ig:ig_petty_bourgeoisie.leader`를 인물 scope로 저장하고 선택지 text에 이름을 표시한다.
- 저장한 소시민 지도자에게 흠차대신의 전권을 주어 광동으로 파견한다.
...
22. `first_opium_war.101.a`는 소시민 이해집단 지도자를 인물 scope로 저장해 이름을 표시하고, 흠차대신으로 광동에 파견한 뒤 `first_opium_war.102`로 이어진다.
