...
|---|---|---|---:|---|
| `character_role_eafp_roju` | 노중 | `politician` | 30 | `{ 7 19 }` |
| `character_role_eafp_rojushuza` | 노중수좌 | `politician` | 35 | `{ 2 10 }` |
| `character_role_eafp_tairo` | 대로 | `politician` | 40 | `{ 0.5 6 }` |
...
`type = politician`을 채택하는 이유는 보직 재임자가 계속 정치인으로 취급되게 하기 위해서다. 바닐라에서도 일반 정치인과 이해집단 지도자가 같은 `politician` 유형의 별도 역할로 정의되어 있다. 다만 새 보직과 일반 정치인 역할의 동시 보유·독립 만료 동작은 아래 사전 검증을 통과해야 한다.
...
character_role_eafp_roju = {
    ...
    career_length = { 7 19 }
    on_career_end = {
        eafp_japan_end_bakufu_office = { POSITION = flag:roju }
    }
}
```
...
7. 저널 활성 여부와 종료 사유에 따라 기존 후임 선출 정책을 적용한다. 노중·노중수좌의 공석은 보충하고, 대로는 기존의 별도 임명 절차를 유지한다.
...
- 사망으로 인한 역할 종료 콜백과 사망 on_action이 둘 다 호출되어도 임무 효과 제거는 한 번만 수행한다. 공석 보충은 저널의 주간 처리로 모아 후임 생성 재진입을 피한다.
...
- 일반 정치인 역할이 먼저 만료되어도 보직과 정치인 취급이 유지되는지.
...
- 임기 종료·사망·명시적 해임의 공석 보충은 저널 주간 처리에서 수행한다. 역할 콜백에서 신규 인물 생성·선출 이벤트를 중첩 실행하지 않는다. 대로는 기존 별도 임명 절차를 유지한다.
