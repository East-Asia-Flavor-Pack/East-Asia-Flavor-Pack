...
4. 조선이 성경, 남만주, 북만주 전역을 소유하면 정책 저널이 완료된다.
5. 완료 효과로 `je_eafp_manchurian_management`가 열린다.
6. 만주 경영 저널은 도시화, 조선인 정착 비율, 세금 행정 조건을 요구한다.
7. 조건이 모두 충족되면 최종 이벤트 `eafp_manchuria.20`이 발생하고 만주 주들이 조선 행정권으로 정착된다.
...
완료 조건은 조선이 만주 핵심 세 주권역을 모두 완전 소유하는 것이다. 완료되면 만주 경영 저널이 추가된다.
